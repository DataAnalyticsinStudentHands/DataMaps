...
Meteor.publish('livedata', function (site, timeChosen) {
	...
	var aggPipe = [
        {$match: {$and: [ {site:site}, 
                {epoch: {$gt: parseInt(timeChosen,10),$lt: parseInt(timeChosen+3600000,10)}}] }}, 
		{ $sort: { epoch : 1} }, 
		{ $project: {subTypes: 1,_id:0}}
	];
	var pollutData = LiveData.aggregate(aggPipe,function (err, line) {
	...
	for (var pubKey in pollData){
		if (pollData.hasOwnProperty(pubKey)){
			// console.log(pubKey)
			self.added('livedata',Random.id(),{pollutKey:{name:pubKey,data:pollData[pubKey]}});
		}
	}
	},
		function (error) {
...
Meteor.publish('aggregatedata5min', function (site,timeChosen) {
	...
	var aggPipe = [{$match: {site:site}}, 
		{ $sort: { epoch : 1} }, 
		{ $project: {O3: 1,_id:0}}
	];
	var pollut5Data = AggrData.aggregate(aggPipe,function (err, line) {
		...
		for (var pubKey in poll5Data){
			if (poll5Data.hasOwnProperty(pubKey)){
			 	//console.log(pubKey)
				self.added('aggregatedata5min',Random.id(),{pollut5Key:{name:pubKey,data:poll5Data[pubKey]}});
			}
		}
	},
	function (error) {
...
Meteor.publish('sites', function(sites4show) {
    return Monitors.find({AQSID:{$in:sites4show}},{AQSID:1,name:1});
});
